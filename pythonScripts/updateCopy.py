...
archiveList = getFicNumList(urlList, runoff) # archiveList = all AO3 ficNums, runoff = all URLs that are NOT AO3 works
...
with open("update/2STORAGE/v8RawData.csv", "a") as newFile:
...
print("\n--- Writing done!! ---\n")
print("\n--- Action Summary ---")
print(f"{len(urlList)} total URLs from input.txt: {len(archiveList)} AO3 works, {dg['run1']} other")
